(ns appfolio.main
  "Kotodama WASM entrypoint for the Appfolio clean-room actor (L4) — Clojure port.
  ...
  facts (`appfolio.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "appfolio")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"appfolio.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "appfolio-compat" :tier tier :entities entities} 200])
...
(defn start! [] :appfolio-compat/ready)
